...
def test_teamnotes_normalized_requirements_keep_provenance_and_conflict_resolution(
    repo_root: Path,
) -> None:
    base = repo_root / "examples/teamnotes"
    req = yaml.safe_load(
        (base / "expected-normalized/requirements.yaml").read_text(encoding="utf-8")
    )
    assert all(
        item["source"]["path"] and item["source"]["location"] for item in req["requirements"]
    )
    conflicts = yaml.safe_load(
        (base / "expected-normalized/conflicts.yaml").read_text(encoding="utf-8")
    )
