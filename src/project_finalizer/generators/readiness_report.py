...
def generate_readiness_report(fs: ProjectFS, graph: ArtifactGraph, report: ReadinessReport, *, output: str, source_ids: tuple[str, ...]) -> ArtifactGraph:
    fs.write_yaml_atomic(output, report.to_mapping())
    return register_generated(fs, graph, artifact_id="READINESS_REPORT", output=output, source_ids=source_ids)
