...
def generate_work_package_graph(
    fs: ProjectFS,
    graph: ArtifactGraph,
    ...
    *,
    ...
    rows = [
        {
            "id": str(item.get("id", "")),
            "depends_on": sorted(str(value) for value in item.get("depends_on", [])),
        }
    ...
    return register_generated(
        fs, graph, artifact_id="WP_GRAPH", output=output, source_ids=source_ids
    )
