...
class AuditLedger:
    ...
    @classmethod
    def from_records(
        cls, *, findings: list[dict[str, Any]], dispositions: list[dict[str, Any]]
    ) -> AuditLedger:
        return cls(
            tuple(dict(item) for item in findings), tuple(dict(item) for item in dispositions)
        )
    ...
    def closure_report(self) -> ValidationReport:
        ...
        for duplicate in sorted({fid for fid in finding_ids if finding_ids.count(fid) > 1}):
            issues.append(
                ValidationIssue(
                    "AUDIT_FINDING_DUPLICATE",
                    f"duplicate finding ID: {duplicate}",
                    "ERROR",
        ...
        for duplicate in sorted({fid for fid in disposition_ids if disposition_ids.count(fid) > 1}):
            issues.append(
                ValidationIssue(
                    "AUDIT_DISPOSITION_DUPLICATE",
                    f"multiple dispositions for finding: {duplicate}",
                    "ERROR",
                    subject_id=duplicate,
                )
            )
        ...
        for fid in sorted(set(disposition_map) - set(finding_map)):
            issues.append(
                ValidationIssue(
                    "AUDIT_ORPHAN_DISPOSITION",
                    f"disposition references unknown finding: {fid}",
                    "ERROR",
                    subject_id=fid,
                )
            )
        for fid in sorted(finding_map):
            ...
            if disposition is None:
                issues.append(
                    ValidationIssue(
                        "AUDIT_DISPOSITION_MISSING",
                        f"finding has no terminal disposition: {fid}",
                        "ERROR",
                        subject_id=fid,
                    )
                )
                if severity == "CRITICAL":
                    issues.append(
                        ValidationIssue(
                            "AUDIT_CRITICAL_OPEN",
                            f"Critical finding remains open: {fid}",
                            "ERROR",
                            subject_id=fid,
                        )
                    )
                    ...
                    issues.append(
                        ValidationIssue(
                            "AUDIT_HIGH_OPEN",
                            f"High finding remains open: {fid}",
                            "ERROR",
                            subject_id=fid,
                        )
                    )
            ...
            if kind not in _TERMINAL:
                issues.append(
                    ValidationIssue(
                        "AUDIT_DISPOSITION_INVALID",
                        f"invalid disposition for {fid}: {kind}",
                        "ERROR",
                        subject_id=fid,
                    )
                )
                continue
            if kind in _EVIDENCE_REQUIRED and (
                not disposition.get("resolution_refs") or not disposition.get("verification_refs")
            ):
                issues.append(
                    ValidationIssue(
                        "AUDIT_DISPOSITION_EVIDENCE_MISSING",
                        f"{kind} disposition requires resolution and verification refs",
                        "ERROR",
                        subject_id=fid,
                    )
                )
            if kind == "DEFERRED" and severity == "CRITICAL":
                issues.append(
                    ValidationIssue(
                        "AUDIT_CRITICAL_OPEN",
                        f"Deferred Critical finding remains a blocker: {fid}",
                        "ERROR",
                        subject_id=fid,
                    )
                )
                ...
                issues.append(
                    ValidationIssue(
                        "AUDIT_HIGH_OPEN",
                        f"Deferred High finding remains a blocker: {fid}",
                        "ERROR",
                        subject_id=fid,
                    )
                )
