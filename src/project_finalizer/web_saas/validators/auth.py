...
class AuthValidator:
    ...
    def validate(self, ctx) -> ValidationReport:
        ...
        if auth.get("credential_transport") == "cookie" and not auth.get("csrf"):
            issues.append(issue("WS_AUTH_CSRF_REQUIRED", "cookie credentials require an explicit CSRF disposition"))
        if auth.get("cross_origin_credentials"):
            ...
            if not isinstance(origins, list) or not origins or "*" in origins:
                issues.append(issue("WS_AUTH_CORS_EXACT_ORIGINS_REQUIRED", "credentialed cross-origin access requires exact origins"))
        if auth.get("credential_transport") and not auth.get("session_or_token_model", True):
            issues.append(issue("WS_AUTH_SESSION_MODEL_REQUIRED", "authentication requires explicit session/token lifecycle semantics"))
