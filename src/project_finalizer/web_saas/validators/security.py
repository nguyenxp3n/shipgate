...
class SecurityValidator:
    ...
    def validate(self, ctx) -> ValidationReport:
        ...
        if enabled(ctx, "files") and not security.get("file_policy"):
            issues.append(
                issue(
                    "WS_SEC_FILE_POLICY_REQUIRED",
                    "enabled file storage requires an upload/storage security policy",
                )
            )
        if enabled(ctx, "webhooks") and not security.get("webhook_policy"):
            issues.append(
                issue(
                    "WS_SEC_WEBHOOK_POLICY_REQUIRED",
                    "enabled webhooks require signature/replay policy",
                )
            )
        ...
        if isinstance(ssrf, dict):
            ...
            if status == "NOT_APPLICABLE" and not evidence:
                issues.append(
                    issue(
                        "WS_SEC_NA_EVIDENCE_REQUIRED",
                        "NOT_APPLICABLE security surfaces require evidence",
                        subject_id="ssrf",
                    )
                )
            if status in {"APPLICABLE", "ENABLED"} and not ssrf.get("policy"):
                issues.append(
                    issue(
                        "WS_SEC_SSRF_POLICY_REQUIRED",
                        "applicable outbound URL surface requires SSRF policy",
                        subject_id="ssrf",
                    )
                )
