...
def build_role_packet(project_root: Path, role_id: str) -> RolePacket:
    ...
    contract_paths = tuple(
        (role_root / name).relative_to(_workflow_root()).as_posix() for name in contract_names
    )
    inputs: list[str] = []
    ...
    inputs.extend(
        ref for ref in role.required_inputs if ref not in missing_required and ref not in inputs
    )
    return RolePacket(
        role_id=role_id,
